...
if($_POST) {
	...
	if($photo_dir) {
		if(!$_POST['count']) {
			...
			$count = $_POST['count'];
			# Process 5 pictures and insert them
			...
			if(!$_SESSION['dir'])
				$_SESSION['dir'] = scandir($large_path);
			
			$first = true;
			$to = $count + 20;
			//$arr_dir = scandir($large_path);
			foreach($_SESSION['dir'] as $key => $file) {
				$uploadFilename = $large_path .'/'. $file;
				$extension = getExtension($file);
				
				if(in_array($extension, $array_extensions) && $key >= $count && $key < $to) {
...
<?php
		...
		if(!$to) $to = 0;
		if($to <= count($_SESSION['dir'])) {
?>
<form name="album_form2" id="album_form2" method="post" action="">
<input type="hidden" name="album_id" value="<?= $album_id ? $album_id : $_POST['album_id']; ?>">
<input type="hidden" name="folder" value="<?= $_POST['folder']; ?>">
<input type="hidden" name="place" value="<?= $_POST['place']; ?>">
<input type="hidden" name="album_title" value="<?= $_POST['album_title']; ?>">
<input type="hidden" name="date" value="<?= $_POST['date']; ?>">
<input type="hidden" name="count" value="<?= $count ? $to : 1; ?>">
</form>
<script language="JavaScript">

	document.album_form2.submit();
