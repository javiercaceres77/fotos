...
function get_albums($user_id) {
	...
	$sql = 'SELECT a.album_id, a.title, a.album_date, a.photo_cover_id, i.file_name, i.img_w, i.img_h, i.path, a.location, l.location_desc, count(ap.photo_id) as num_photos
FROM albums a
INNER JOIN users_albums ub ON ub.album_id = a.album_id
INNER JOIN album_photo ap ON ap.album_id = a.album_id
LEFT JOIN images i ON i.photo_id = a.photo_cover_id AND i.img_type = \'thumb\'
LEFT JOIN locations l ON l.location_id = a.location
WHERE user_id = '. $user_id .'
GROUP BY a.album_id, a.title, a.album_date, a.photo_cover_id, i.file_name, i.img_w, i.img_h, i.path, a.location, l.location_desc
ORDER BY a.album_date DESC';

	$select_album_details = my_query($sql, $conex);
	$albums_arr = array();
	while($record = my_fetch_array($select_album_details))
		$albums_arr[] = $record;
	
	# get number of pics per album
